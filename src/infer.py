...
def plot_three_way_comparison(true_coords, base_coords, ref_coords, title="Comparison", filename="plot.html"):
    """
    Plots three aligned protein backbones in the same interactive viewer,
    offsetting them along the X-axis so they sit side-by-side without overlapping.
    """
    try:
        import plotly.graph_objects as go
    except ImportError:
        print("[WARNING] Plotly not installed. Skipping HTML visualization. (pip install plotly)")
        return

    # Calculate dynamic offset based on the maximum width of the true protein
    # We multiply by 2 to give comfortable breathing room between the models
    offset_val = np.max(np.abs(true_coords[:, 0])) * 2.0 + 15.0

    # Apply physical translation along the X-axis
    offset_base = np.array([-offset_val, 0, 0])  # Shift Base to the Left
    offset_ref = np.array([offset_val, 0, 0])    # Shift Refined to the Right
    ...
    # 3. L-BFGS Refined (Right)
    fig.add_trace(go.Scatter3d(
        x=ref_shifted[:, 0], y=ref_shifted[:, 1], z=ref_shifted[:, 2],
        mode='lines+markers',
        marker=dict(size=4, color='lightgreen'),
    ...
    fig.update_layout(
        ...
        scene=dict(
            ...
            yaxis_title='Y',
            zaxis_title='Z',
            aspectmode='data' # CRITICAL: Ensures 1:1:1 physical scaling so it doesn't stretch
        ),
