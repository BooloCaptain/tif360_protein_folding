...
def _init_gaussian_geometry_bias(out_layer: nn.Linear) -> None:
    ...
    # log_var = 0.0 initializes the variance to 1.0 (e^0)
...
class FrozenESMEmbedder(nn.Module):
    """Loads ESM-2 650M, completely freezes it, and extracts 1D + 2D representations."""
    def __init__(self, d_model=256, extract_attentions=True):
        super().__init__()
    ...
    def forward(self, tokens):
        ...
        self.esm_model.eval()
        with torch.no_grad():
            # Extract representations AND attention weights
            ...
            if self.extract_attentions:
                # Shape is [B, 33, 20, L+2, L+2] regardless of repr_layers
                attentions = results["attentions"] 
                
                # BUG 2 FIXED: Explicitly slice the last 6 layers (6 layers * 20 heads = 120 channels)
...
class GaussianGeometryHead(nn.Module):
    """Predicts means and log-variances for kinematic geometry."""
...
class EarlyBranchingConfidenceNetwork(nn.Module):
    ...
    ESM-2 attention maps injected safely into the 2D track via Outer Product.
    """
    def __init__(
        self,
        d_model=256,
        ...
        # 1. Embedder (now extracting attention)
        self.embedder = FrozenESMEmbedder(d_model=d_model, extract_attentions=True)
        ...
        # --- ESM-2 Attention Map Compressor ---
        self.attn_map_proj = nn.Linear(120, d_pair, bias=False)
        # Let PyTorch use default initialization here!
        
        # NEW: A dedicated LayerNorm to rescue the vanishing 1/L Softmax signal
    ...
    def forward(self, tokens, src_key_padding_mask=None):
        ...
        # 1. Extract raw, stable embeddings (std ~ 1.0)
        x_raw, esm_attentions = self.embedder(tokens)
        x = self.proj(x_raw)
        
        # ==========================================
        # BRANCH A: THE 2D PAIR TRACK 
        # (Uses x_raw to prevent Outer Product Variance Explosion)
        # ==========================================
        left = self.pair_proj_left(x_raw)   
        right = self.pair_proj_right(x_raw) 
        
        pair_track = left.unsqueeze(2) + right.unsqueeze(1)
        product = (left.unsqueeze(2) * right.unsqueeze(1)) / math.sqrt(self.d_pair)
        pair_track = pair_track + product
        
        if esm_attentions is not None:
            esm_attentions = (esm_attentions + esm_attentions.transpose(1, 2)) / 2.0
            projected_attn = self.attn_map_proj(esm_attentions)
            projected_attn = self.attn_norm(projected_attn)
            pair_track = projected_attn
            
        positions = torch.arange(L, device=x_raw.device)
        ...
        # ==========================================
        # BRANCH B: THE 1D TRANSFORMER BACKBONE
        # (Uses x_scaled to preserve Residual Identity and Softmax Sharpness)
        # ==========================================
        x = x * math.sqrt(self.d_model) # RESTORED!
        ...
        # 4. Pure 1D Transformer Loop (Now safely using the scaled `x`)
        cos = self.rope_cos[:L]
        sin = self.rope_sin[:L]
        for layer in self.layers:
            x = checkpoint(layer, x, cos, sin, padding_mask_bool, use_reentrant=False)

        # 5. Distogram Prediction
        disto_logits = self.disto_head(pair_track)
        d_probs = F.softmax(disto_logits, dim=-1)
        
        # 6. Spatial Context & Geometry
        # Because `x` is scaled again, `disto_query(x)` will produce strong logits,
        # perfectly restoring the SpatialAttentionPooler!
