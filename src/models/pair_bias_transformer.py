...
# ==========================================
# 3. BACKBONE (PAIR BIAS SPECIFIC)
# ==========================================
class PairBiasTransformerBlock(nn.Module):
    """1D Transformer that is biased by the 2D track, but does NOT update the 2D track."""
    def __init__(self, d_model=256, nhead=8, dim_feedforward=1024, dropout=0.1, d_pair=64):
        ...
        # Reads the 2D track to create an attention mask
    ...
    def forward(self, x, pair_track, cos, sin, padding_mask_bool=None):
        ...
        # Extract 2D structural bias
        normalized_pair = self.pair_bias_norm(pair_track)
        pair_bias = self.pair_to_bias(normalized_pair).permute(0, 3, 1, 2)
        
        # [THE FIX]: Clamp the SDPA bias to safely bypass bfloat16 Softmax Overflow
...
class SpatialAttentionPooler(nn.Module):
    ...
    def forward(self, h, disto_logits, valid_keys_mask=None):
        ...
        # [THE FIX]: Clamp the custom Spatial Attention queries
...
# ==========================================
class PairBiasNetwork(nn.Module):
    """
    A hardcoded, highly explainable model representing the optimized configuration:
    - esm_mode: unfreeze_last_n: 0
    - block_type: pair_bias
    - pair_context_to_head: true
    - head_mode: hierarchical_ss_disto (with spatial attention)
    """
    def __init__(
        self,
        d_model=256,
        ...
        for block in self.layers:
            block.pair_to_bias.weight.data *= 0.01
    ...
    def forward(self, tokens, src_key_padding_mask=None):
        ...
        # 2. Deep Transformer Loop (pair_track provides mask, but is not updated)
        for layer in self.layers:
            x = checkpoint(layer, x, pair_track, cos, sin, padding_mask_bool, use_reentrant=False)
        
        # [THE FIX]: Apply the final 1D normalization before passing the residual stream to the output heads
        x = self.final_1d_norm(x)

        left_final = self.disto_proj_left(x).unsqueeze(2)
        right_final = self.disto_proj_right(x).unsqueeze(1)
        pair_track = left_final + right_final
        
        # 4. Distogram & Geometry Prediction
